...
func _process(delta: float) -> void:
	for i in range(current_special_deals.size() - 1, -1, -1):
		if current_special_deals[i].deal_time <= 0:
			if current_special_deals[i].for_player_claim:
				continue
			if current_special_deals[i].player_bid >= current_special_deals[i].highest_bid:
				current_special_deals[i].for_player_claim = true
...
func try_to_bid() -> void:
	for d in current_special_deals:
		if d.deal_time <= 10.0:
			continue
		if d.highest_bid <= d.lootbox_data.lootbox_price * 3 + (PlayerInventory.renown / 2.0):
			var will_bid : bool = 1 == randi_range(0,3)
			if will_bid:
				d.highest_bid = d.highest_bid + d.BID_INCREASE
				PlayerInventory.earn_money(d.player_bid)
