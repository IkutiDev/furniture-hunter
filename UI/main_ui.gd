...
@export var button_text_during_closing : String

@export var money_label : Label
@export var renown_label : Label
@export var hour_label : Label

@export var cash_register_menu : CashRegisterUI
...
@export var cash_register_button : TextureButton
...
func _ready() -> void:
	...
	cash_register_button.pressed.connect(on_cash_register_button_pressed)
...
func close_all_menu_except(exception = Control.new()):
	var all_menus = [cash_register_menu,inventory_menu,crafting_menu,auctions_menu]
	if all_menus.has(exception):
		all_menus.erase(exception)
...
func disable_all_buttons() -> void:
	...
	cash_register_button.texture_normal = disactive_button_texture
...
func on_inventory_button_pressed() -> void:
	close_all_menu_except(inventory_menu)
...
func on_cash_register_button_pressed() -> void:
	close_all_menu_except(cash_register_menu)


	disable_all_buttons()
	
	if not cash_register_menu.visible:
		cash_register_menu.open_menu()
		cash_register_button.texture_normal = active_button_texture
	else:
		cash_register_button.texture_normal = disactive_button_texture
		cash_register_menu.close_menu()
	pass


func on_collections_button_pressed() -> void:
	close_all_menu_except(crafting_menu)
...
func on_keys_button_pressed() -> void:
	close_all_menu_except(auctions_menu)
...
func on_game_state_changed() -> void:
	match GameManager.game_state:
		...
		GameManager.GameState.ENDING_DAY:
			change_shop_state_button.text = button_text_during_closing
			#change_shop_state_button.disabled = true
